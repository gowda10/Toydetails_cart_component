package com.kidzoo.toydetails.integration.test;
...
@SpringBootTest
class ToydetailsIntegrationTest {
	...
	void getToyDetailsTest() {
		try {
			...
			given(toyDetailsController.getToyDetails()).willReturn(toyDetailsResponse);
	...
	void getToyDetailsBasedOnPriceRangeTest(){
		try{
			...
			ToyDetailsResponse toyDetailsResponse = mapper.readValue(toyDetails, ToyDetailsResponse.class);
			given(toyDetailsController.getToyDetailsBasedOnPriceRange(ArgumentMatchers.any(),ArgumentMatchers.any())).willReturn(toyDetailsResponse);
	...
	void getToyStatusByIdTest() {
		try {
			...
			given(toyDetailsController.getToyStatusById(ArgumentMatchers.anyInt())).willReturn(toyStatusById);
		...
		void getToyDetailsByStatusAvailableTest() {
			try {
				...
				given(toyDetailsController.getToyListByStatus(ArgumentMatchers.any())).willReturn(toyDetailsResponse);
				mockMvc.perform(get("/v1/find-Toys-By-Status").queryParam("status","available")
						.contentType(MediaType.APPLICATION_JSON)).andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
			} catch (Exception e) {
			}
